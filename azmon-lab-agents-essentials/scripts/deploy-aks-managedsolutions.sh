...
az monitor account create -g $RESOURCE_GROUP -n $PROM_NAME 
...
az grafana create --resource-group $RESOURCE_GROUP --name $MANAGED_GRAFANA
...
prometheusId=$(az monitor account show --resource-group $RESOURCE_GROUP -n $PROM_NAME --query id -o tsv)
...
grafanaId=$(az grafana show --resource-group $RESOURCE_GROUP -n $MANAGED_GRAFANA --query id -o tsv)
...
az aks update --enable-azure-monitor-metrics -n $AKS_CLUSTER -g $RESOURCE_GROUP --azure-monitor-workspace-resource-id $prometheusId --grafana-resource-id $grafanaId
